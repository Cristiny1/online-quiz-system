...
// Page title
$dashboardTitle = $userRole === 'admin' ? 'Admin' : ($userRole === 'teacher' ? 'Teacher' : 'Student') . ' Dashboard';
...
try {
    if ($userRole === 'admin') {
        ...
        $newUsers = $pdo->query("SELECT COUNT(*) FROM users WHERE MONTH(created_at) = MONTH(CURRENT_DATE())")->fetchColumn();
        ...
        $stmt = $pdo->prepare("SELECT COALESCE(AVG(score), 0) FROM attempts WHERE user_id = ?");
        $stmt->execute([$userId]);
        $avgScore = round($stmt->fetchColumn(), 1);
        
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM quizzes WHERE status='published'");
        $stmt->execute();
        $availableQuizzes = $stmt->fetchColumn();
        ...
        $stmt = $pdo->prepare("
            ...
            AND q.id NOT IN (
                SELECT quiz_id FROM attempts WHERE user_id = ?
            )
            ORDER BY q.created_at DESC
            LIMIT 5
        ");
        $stmt->execute([$userId]);
        $newQuizzes = $stmt->fetchAll();
    }
    ...
    if ($userRole === 'admin') {
        ...
        $stmt = $pdo->prepare("
            SELECT 'attempt' AS type, CONCAT('You scored ', score, '% on \"', q.title, '\"') AS message, a.created_at
            FROM attempts a
            JOIN quizzes q ON a.quiz_id = q.id
            WHERE a.user_id = ?
            ORDER BY a.created_at DESC
    ...
    if ($userRole === 'admin') {
        ...
        $totalAttempts = $avgScore = $availableQuizzes = 0;
        $recentAttempts = $newQuizzes = [];
    }
    $notifications = [];
}
?>
...
<div class="sidebar" id="sidebar">
    ...
    <?php if ($userRole === 'student'): ?>
        <a href="available-quizzes.php" class="nav-link">
            <i class="fas fa-play-circle"></i> Take Quiz
        </a>
        <a href="my-results.php" class="nav-link">
...
<div class="main-content">
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-value"><?= $availableQuizzes ?></div>
        ...
        <div class="col-md-4 col-sm-6">
            <div class="stat-card">
                ...
                <div class="stat-trend <?= $avgScore >= 70 ? 'up' : 'down' ?>">
                    <i class="fas fa-chart-line"></i>
                    <?= $avgScore >= 70 ? 'Good job!' : 'Keep practicing' ?>
                </div>
            </div>
        </div>
        <?php endif; ?>
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            ...
            <?php if ($userRole === 'student' && !empty($newQuizzes)): ?>
            <div class="activity-card mt-4">
                <div class="card-header">
                    <i class="fas fa-clock"></i> New Quizzes Available
                </div>
                <div class="card-body">
                    ...
                    <div class="list-item">
                        ...
                        <div class="list-item-time">
                            <a href="take-quiz.php?id=<?= $quiz['id'] ?>" class="btn btn-sm btn-primary">
        ...
        <div class="col-lg-4">
            ...
            <div class="activity-card mb-4">
                ...
                <div class="card-body">
                    <div class="quick-actions">
                        <?php if ($userRole === 'teacher'): ?>
                        <a href="create-quiz.php" class="quick-action-btn">
                        ...
                        <?php if ($userRole === 'student'): ?>
                        <a href="available-quizzes.php" class="quick-action-btn">
                        ...
                        <a href="setting.php" class="quick-action-btn">
            ...
            <!-- Recent Results (for students) -->
            <?php if ($userRole === 'student' && !empty($recentAttempts)): ?>
            <div class="activity-card mt-4">
                <div class="card-header">
                ...
                <div class="card-body">
                    ...
                    <div class="list-item">
                        ...
                        <div class="list-item-content">
                            <div class="list-item-title"><?= htmlspecialchars($attempt['quiz_title']) ?></div>
                            <div class="list-item-subtitle">
                                Score: <strong><?= round($attempt['score']) ?>%</strong>
                            </div>
                        </div>
                        <div class="list-item-time">
                            <?= date('M d', strtotime($attempt['created_at'])) ?>
                        </div>
                    </div>
                    <?php endforeach; ?>
                </div>
